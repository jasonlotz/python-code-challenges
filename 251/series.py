...
def basic_series() -> pd.Series:
    ...
    """
    return pd.Series([1, 2, 3, 4, 5], dtype='int64', name='Fred')
...
def float_series() -> pd.Series:
    ...
    """
    data = [x for x in np.arange(0, 1.0001, 0.001)]
...
def alpha_index_series() -> pd.Series:
    ...
    """
    data = [x for x in np.arange(1, 27)]
    indices = [i for i in string.ascii_lowercase]

    return pd.Series(data, dtype='int64', index=indices)
...
def object_values_series() -> pd.Series:
    ...
    Don't worry about the series name.
    """
    data = [i for i in string.ascii_uppercase]
    indices = [x for x in np.arange(101, 127)]

    return pd.Series(data, dtype='object', index=indices)
